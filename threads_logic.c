...
int	monitor_utils(t_philo *philos)
{
	...
	while (i < philos->main_st->number_of_philosophers)
	{
		...
		if (ft_time() - philos[i].last_eat_time
			>= philos[i].main_st->time_to_die)
		{
			pthread_mutex_lock(&philos->main_st->is_died_mutex);
			pthread_mutex_lock(&philos->main_st->print);
			printf("%ld %d %s\n", ft_time() - philos->main_st->start,
				philos[i].id, "died");
...
void	*monitor_thread(void *arg)
{
	...
	while (1)
	{
		...
		if (philos->main_st->number_of_times_each_philo_must_eat != -1
